...
class MultiSpeakerASRstreamer:
    ...
    def perform_streaming_stt_spk(
        ...
        (
            ...
            fifo_preds,
            diar_pred_out_stream
        ) = self.asr_model.diar_conformer_stream_step(
            step_num,
            ...
            previous_diar_pred_out=diar_pred_out_stream,
...
def perform_streaming(cfg, asr_model, diar_model, bsd_spk, streaming_buffer, debug_mode=False):
    ...
    data[0]['pred_text'] = transcribed_texts[0].text
...
def main(cfg: DiarizationConfig) -> Union[DiarizationConfig]:
    ...
    #transcribing configuration override
    ...
    orig_config = imported_class.restore_from(
        restore_path=cfg.asr_model, map_location=map_location,
        return_config=True
    )
    orig_config.spk_supervision_strategy = 'diar' #change spk supervision to diar_preds
    orig_config.binarize_diar_preds_threshold = False #disable binarize diar_preds
    orig_config.diar_model_path = cfg.diar_model_path #xwchange diar_model_path
    new_config = orig_config
    asr_model = imported_class.restore_from(
        restore_path=cfg.asr_model, strict = False, map_location=map_location, override_config_path=new_config
    )
    # import ipdb; ipdb.set_trace()
    asr_model.encoder.setup_streaming_params(att_context_size=[188, 13])
    ...
    asr_model.diarization_model = diar_model
