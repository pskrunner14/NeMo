...
class TestSortingUtils:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (torch.cat([torch.zeros(1, 499), torch.tensor([[0.6]]), torch.zeros(1, 500)], dim=1), -1, 0.5, torch.tensor([499])),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                torch.tensor([[0.1, 0.4, 0.9, 0.5], [0.6, 0.3, 0.7, 0.2]]),  # match_score (batch_size=2, num_permutations=4)
                torch.tensor([[0, 1, 2], [1, 0, 2], [2, 1, 0], [1, 2, 0]]),  # speaker_permutations (num_permutations=4, num_speakers=3)
    ...
    @pytest.mark.parametrize("batch_size, num_frames, num_speakers", [
    ...
    @pytest.mark.parametrize("labels, batch_perm_inds, expected_output", [
