...
from nemo.collections.asr.data.audio_to_eesd_label import (
    extract_seg_info_from_rttm,
    get_frame_targets_from_rttm,
    get_global_seg_spk_labels,
    get_speaker_labels_from_diar_rttms,
)
...
class AudioToEESDLabelDataset(Dataset):
    ...
    def __init__(
        ...
        self.collection = DiarizationSpeechLabel(
            manifests_files=manifest_filepath, emb_dict=None, clus_label_dict=None, pairwise_infer=False,
        )
    ...
    def __getitem__(self, index: int) -> EESDAudioRTTMItem:
        ...
        audio_signal = self.featurizer.process(
            sample.audio_file,
            offset=offset,
            duration=duration,
            trim=self.trim,
        ...
        session_len_samples = (
            int(self.session_len_sec * self.featurizer.sample_rate) if self.session_len_sec else audio_signal.size(0)
        )
        ...
        if self.session_len_sec and self.random_offset and curr_duration > self.session_len_sec:
            ...
            uniq_id = (
                f"{uniq_id}_RandOffset{str(int(round(random_offset, self.round_digits) * pow(10, self.round_digits)))}"
            )
        ...
        labels, speaker_id_map = self.parse_rttm_for_frame_labels(
            uniq_id, sample.rttm_file, total_offset, total_duration
        )
...
def get_audio_to_eesd_label_dataset_from_config(
    config: Dict, local_rank: int, global_rank: int, world_size: int,
